...
The system consists of a main 2D rectangular channel of length $L=1~m$ (from $x=-0.2$ to $0.8$) and width $d = 0.4~m$. Inside the channel, there is a rectangular obstacle of dimensions $0.4~m \times 0.1~m$ starting at $x=0$. The fluid has density $\rho = 1~kg/m^3$ and we have studied the case $\nu=0.01~m^2/s$. 

The boundary conditions are the following: 
- At the entrance (inlet), $u=u_{\text{inlet}}$ and $v=0$.
- At the exit (outlet), $p=0$.
- At the walls (including the rectangular obstacle boundaries), $u=v=0$.
...
We define the partial differential equations at the `pde(X,Y)` function, and use the DeepXDE library to build the Physics-Informed Neural Network (PINN). The geometry is defined using Constructive Solid Geometry (CSG) by subtracting the rectangular obstacle from the main channel domain.

To train the model properly, we employ several advanced strategies:

1. **Point Distribution & Anchors:** We sample $4000$ points inside the domain and $1500$ at the boundaries. Because sharp corners create complex flow dynamics, we explicitly force the network to learn the physics around the obstacle by adding $3000$ extra anchor points. These anchors are dynamically generated within a $0.05~m$ buffer zone around the four edges of the rectangle.
2. **Custom Loss Weights:** We assign specific weights to different components of the loss function. The boundary conditions of the obstacle and the channel walls are penalized more heavily ($w=20.0$ and $w=5.0$, respectively) compared to the PDE residual ($w=1.0$) to enforce strict adherence to the no-slip conditions.
3. **Multi-Stage Training:** The network is trained in three phases:
   ...
   - L-BFGS optimizer for final convergence and high-precision fine-tuning.